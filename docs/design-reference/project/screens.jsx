...
function Avatar({ name, size = "md", you = false }) {
  ...
  return (
    <div
      className={cls}
      style={you ? { background: "var(--white)", color: "var(--black)", borderColor: "var(--white)" } : null}
      title={name}
    >
      {initial}
    </div>
    ...
    <div className="scene">
      <div className="home-wrap fade-in">
        ...
        <p className="home-lede">
          Cards Against Bhayanak is an original party game where one player
          reads a prompt and everyone else submits the funniest, worst,
          most morally indefensible answer. Then someone gets a point.
        </p>
        ...
        <div className="home-marquee">
          <div className="home-marquee-track">
            {Array.from({length:2}).flatMap((_, k) => [
              "Free to play", "·", "Up to 10 players", "·", "6 card packs", "·",
              "House rules supported", "·", "No download", "·", "Designed for chaos", "·"
...
function CreateScreen({ go, draft, setDraft }) {
  ...
  const cardCount = window.CARD_PACKS
  ...
  return (
    <div className="scene">
      <Topbar
        onHome={() => go("home")}
        right={
          <button className="btn btn-ghost btn-sm" onClick={() => go("home")}>← Back</button>
        }
      />
      <div className="create-wrap fade-in">
        <div className="create-hd">
          ...
          <p className="create-sub">Set your handle, pick your packs, and turn on the house rules you can live with.</p>
        </div>
        ...
        <div className="create-grid">
          ...
          <div className="create-col">
            <div className="sheet">
              <div className="sheet-hd">
                <div>
                  ...
                  <div className="sheet-sub">This is how everyone in the lobby will see you. Keep it short.</div>
                </div>
              </div>
            ...
            <div className="sheet">
              <div className="sheet-hd">
                <div>
                  ...
                  <div className="sheet-sub">You can change these any time before the game starts.</div>
                </div>
              </div>
              <div className="opt-grid">
                <div className="opt-row">
                  ...
                  <div className="stepper">
                    <button className="stepper-btn" disabled={draft.maxPlayers <= 3} onClick={() => setDraft(d => ({...d, maxPlayers: d.maxPlayers - 1}))}>−</button>
                    <div className="stepper-val">{draft.maxPlayers}</div>
                    <button className="stepper-btn" disabled={draft.maxPlayers >= 10} onClick={() => setDraft(d => ({...d, maxPlayers: d.maxPlayers + 1}))}>+</button>
                  </div>
                </div>
                <hr className="hr" />
                <div className="opt-row">
                  ...
                  <div className="stepper">
                    <button className="stepper-btn" disabled={draft.roundsToWin <= 3} onClick={() => setDraft(d => ({...d, roundsToWin: d.roundsToWin - 1}))}>−</button>
                    <div className="stepper-val">{draft.roundsToWin}</div>
                    <button className="stepper-btn" disabled={draft.roundsToWin >= 20} onClick={() => setDraft(d => ({...d, roundsToWin: d.roundsToWin + 1}))}>+</button>
                  </div>
                </div>
                <hr className="hr" />
                <div className="opt-row">
            ...
            <div className="sheet">
              <div className="sheet-hd">
                ...
                <div className="pill"><span className="dot"/>{cardCount.toLocaleString()} cards</div>
              </div>
            ...
            <div className="sheet">
              <div className="sheet-hd">
                <div>
                  ...
                  <div className="sheet-sub">Optional. Toggle on the ones your group has actually agreed to.</div>
                </div>
              </div>
              <div className="rule-grid">
                {window.HOUSE_RULES.map(r => {
                  ...
                  return (
                    <div key={r.id} className={`check-card ${on ? "on" : ""}`} onClick={() => toggleRule(r.id)}>
                      <div className="check-box" />
                      <div className="grow">
          ...
          <aside className="create-aside">
            <div className="sheet" style={{position:"sticky", top: 20}}>
              ...
              <div className="summary-row"><span>Host</span><b>{draft.username || "—"}</b></div>
              <div className="summary-row"><span>Players</span><b>{draft.maxPlayers}</b></div>
              <div className="summary-row"><span>Score to win</span><b>{draft.roundsToWin}</b></div>
              <div className="summary-row"><span>Round timer</span><b>{draft.timer}</b></div>
              <div className="summary-row"><span>Packs</span><b>{draft.packs.length}</b></div>
              <div className="summary-row"><span>House rules</span><b>{draft.rules.length}</b></div>
              <div className="summary-row"><span>Card pool</span><b>{cardCount.toLocaleString()}</b></div>
              ...
              <button className="btn btn-ghost btn-block btn-sm" onClick={() => go("home")} style={{marginTop: 8}}>
                Cancel
              </button>
              {!canStart && (
...
function LobbyScreen({ go, draft, isHost, asSpectator = false, roomFull = false }) {
  ...
  const startingPlayers = roomFull
    ? [...window.LOBBY_PLAYERS, { name: "Aanya", avatar: "A", host: false, ready: true,  you: false }].slice(0, draft.maxPlayers)
  ...
  useEffect(() => {
    if (roomFull) return;
    const t = setTimeout(() => {
      setPlayers(p => p.length < draft.maxPlayers ? [...p, { name: "Aanya", avatar: "A", host: false, ready: false, you: false }] : p);
    }, 4500);
  ...
  const cardCount = window.CARD_PACKS
    .filter(p => draft.packs.includes(p.id))
    .reduce((s, p) => s + p.count, 0);
  const packNames = window.CARD_PACKS
    .filter(p => draft.packs.includes(p.id))
    .map(p => p.name);
  ...
  return (
    <div className="scene">
      <Topbar
        ...
        right={
          <>
            <div className="pill"><span className="dot live"/>LIVE LOBBY</div>
            <button className="btn btn-ghost btn-sm" onClick={() => go("home")}>Leave</button>
          </>
        }
      />
      <div className="lobby-wrap fade-in">
        <div className="lobby-hd">
          <div>
            <div className="eyebrow">{asSpectator ? "Lobby · Joined as spectator" : "Lobby · Waiting for players"}</div>
            <h1 className="lobby-title">
              {asSpectator
                ? <>You're in.<br/>The host hasn't started yet.</>
                : <>Share the code.<br/>Wait for your worst friends.</>}
            </h1>
          </div>
        ...
        <div className="lobby-grid">
            ...
            <div className="sheet-hd">
              <div>
                ...
                <div className="sheet-sub">
                  ...
                  {players.length >= draft.maxPlayers ? " · seats full" : " · need at least 3 to start"}
                </div>
              </div>
              <div className="pill">
                <span className="dot"/>
                {players.length >= draft.maxPlayers ? "FULL" : `${players.filter(p => p.ready).length} READY`}
              </div>
            </div>
            ...
            <div className="player-list">
                ...
                <div key={i} className="player-row deal" style={{ animationDelay: `${i * 0.05}s` }}>
                  ...
                  <div className="grow">
                    ...
                    <div className="player-meta">
                      {p.ready ? "Ready" : "Choosing a card back…"}
                    </div>
                  </div>
              ...
              {Array.from({ length: empties }).map((_, i) => (
                <div key={`e${i}`} className="player-row player-row-empty">
                  <div className="avatar avatar-lg" style={{background: "transparent", border: "1px dashed var(--hairline-2)", color: "var(--gray-4)"}}>?</div>
                  <div className="grow">
            ...
            <div className="spectator-block">
              <div className="row" style={{justifyContent: "space-between", marginBottom: 10}}>
                ...
                {players.length >= draft.maxPlayers && !asSpectator && (
                  <div className="muted" style={{fontSize: 11}}>Seats are full — new joiners auto-spectate.</div>
                )}
              </div>
          ...
          <div className="col" style={{gap: 16}}>
            <div className="sheet">
              <div className="eyebrow" style={{marginBottom: 10}}>Game</div>
              <div className="summary-row"><span>Host</span><b>{players[0]?.name}</b></div>
              <div className="summary-row"><span>Score to win</span><b>{draft.roundsToWin} points</b></div>
              <div className="summary-row"><span>Round timer</span><b>{draft.timer}</b></div>
              <div className="summary-row"><span>Room access</span><b>Code only</b></div>
            </div>

            <div className="sheet">
              <div className="eyebrow" style={{marginBottom: 10}}>Active packs · {cardCount.toLocaleString()} cards</div>
            ...
            {asSpectator ? (
              <div className="sheet spectator-cta">
                ...
                <div>
                  <div style={{fontWeight: 700, fontSize: 15, marginBottom: 4}}>You're a spectator</div>
                  <div className="muted" style={{fontSize: 12, lineHeight: 1.5}}>
                    You'll see every card as it's revealed, but you can't play one. If a player drops, you'll be offered the seat.
                  </div>
                </div>
              </div>
            ) : isHost ? (
              ...
              <div className="sheet" style={{textAlign: "center", padding: 20}}>
                ...
                <div className="loading-dots" style={{marginTop: 8}}><span/><span/><span/></div>
              </div>
            )}
          </div>
        </div>
...
function GameScreen({ go, draft, role, setRole }) {
  ...
  const hand = useMemo(() => {
    ...
    return Array.from({ length: 7 }).map((_, i) => window.RESPONSE_CARDS[(offset + i) % window.RESPONSE_CARDS.length]);
  }, [round]);

  // simulated submissions (face-down for picking phase / face-up for judging)
  const submissions = useMemo(() => {
    const list = window.PLAYERS
      .filter(p => p.name !== judge.name)
      .map((p, i) => ({
        player: p,
        // each submission has `blanks` fills
        fills: Array.from({ length: blanks }).map((_, b) =>
          window.RESPONSE_CARDS[(i * 3 + round * 5 + b * 7) % window.RESPONSE_CARDS.length]
        )
      }));
    return list;
      ...
      <Topbar
        ...
        right={
          <>
            ...
            <div className="pill"><span className="dot live"/>{draft.timer === "Off" ? "NO TIMER" : draft.timer.toUpperCase()}</div>
            <button className="btn btn-ghost btn-sm" onClick={() => go("home")}>Leave</button>
          </>
        }
      />
      ...
      <div className="game-wrap">
        ...
        <div className="scoreboard">
          {window.PLAYERS.map((p, i) => {
            ...
            return (
              <div key={p.name} className={`score-chip ${p.you ? "you" : ""} ${isCurrentJudge ? "is-judge" : ""}`}>
                <Avatar name={p.name} size="sm" you={p.you} />
                <div className="col" style={{gap: 1}}>
                  <div className="score-name">{p.name === "You" ? draft.username || "You" : p.name}</div>
        ...
        <div className="row" style={{justifyContent: "flex-end", marginBottom: 12}}>
          <div className="role-toggle">
            <span className="muted" style={{fontSize: 11, marginRight: 8}}>DEMO ROLE</span>
            <div className="seg">
              <button className={`seg-btn ${role === "player" ? "active" : ""}`} onClick={() => setRole("player")}>Player</button>
              <button className={`seg-btn ${role === "judge" ? "active" : ""}`} onClick={() => setRole("judge")}>Judge</button>
            </div>
          </div>
        </div>
        ...
        {(phase === "picking" || phase === "waiting") ? (
          <div className="stage stage-solo">
            <div className="stage-prompt stage-prompt-hero">
              <div className="eyebrow" style={{marginBottom: 12}}>
                The prompt · {judge.name === "You" ? draft.username || "You" : judge.name} is judging
              </div>
              <PromptCard size="xl" text={prompt.text} />
              <div className="pick-status">
                {phase === "picking" && !isJudge && (
                  blanks > 1
                    ? <span>Pick <b>{blanks}</b> cards in order ({selected.length}/{blanks}).</span>
                    : selected.length === 0
                      ? <span>Pick a card from your hand.</span>
                      : <span>Card selected. Hit submit when you're ready.</span>
                )}
                {phase === "picking" && isJudge && (
                  <span>You're the judge. Waiting for everyone to play…<span className="loading-dots" style={{marginLeft: 8}}><span/><span/><span/></span></span>
                )}
                {phase === "waiting" && (
                  <span>Card submitted. Waiting on others…<span className="loading-dots" style={{marginLeft: 8}}><span/><span/><span/></span></span>
                )}
              </div>
              ...
              <div className="pick-progress">
                {submissions.map((s, i) => {
                  ...
                  return (
                    <div key={i} className={`pick-pip ${submitted ? "on" : ""}`} title={s.player.name}>
                      <span className="pick-pip-letter">{s.player.avatar}</span>
                    </div>
                  );
                })}
                <div className="pick-progress-label muted">
                  {phase === "waiting" ? `${submissions.length} of ${submissions.length} submitted` : `0 of ${submissions.length} submitted`}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="stage">
            ...
            <div className="stage-prompt">
              ...
              />
              {phase === "reveal" && (
                <div className="muted" style={{marginTop: 16, fontSize: 13, textAlign: "center", maxWidth: 320}}>
                  {revealIndex < submissions.length ? `Revealing ${revealIndex + 1} of ${submissions.length}…` : "Pick your winner."}
                </div>
              )}
            </div>
            ...
            <div className="stage-subs">
              ...
              {phase === "judging" && (
                <div className="subs-grid subs-grid-large">
                  ...
                  {!isJudge && (
                    <div className="judge-note">Judge is reading. Hold tight.</div>
                  )}
                  {isJudge && (
                    <div className="judge-note">Click any card to start the reveal.</div>
                  )}
                </div>
              )}
                            ...
                            {isWinner && fi === 0 && (
                              <div className="winner-badge">
                                <div className="winner-by">
                                  ...
                                  <span>+1 {s.player.name === "You" ? draft.username || "You" : s.player.name}</span>
                                </div>
                              </div>
                            )}
                          </div>
          ...
          <div className="hand-dock-hd">
            ...
            >
              {selected.length < blanks
                ? (blanks > 1 ? `Pick ${blanks - selected.length} more` : "Pick a card")
                : (blanks > 1 ? "Submit cards →" : "Submit card →")}
            </button>
          </div>
          <div className="hand">
            {hand.map((text, i) => {
        ...
        {phase === "judging" && isJudge && (
          <div className="judge-bar">
            ...
            <button className="btn btn-primary btn-sm" onClick={startReveal}>Start reveal →</button>
          </div>
        )}
      </div>
    </div>
      ...
      <div className="join-wrap fade-in">
        ...
        <div className="sheet" style={{width: "100%", maxWidth: 480, marginTop: 8}}>
          <div className="field">
            ...
            <input
              className="input"
              style={{fontFamily: "var(--font-mono)", letterSpacing: "0.16em", fontSize: 22, textAlign: "center", textTransform: "uppercase"}}
              placeholder="B7K-9MV"
              maxLength={8}
              value={code}
          ...
          <div className="field" style={{marginTop: 16}}>
            ...
            {roomFull ? (
              <div className="join-full-banner">
                ...
                <div className="grow">
                  <div style={{fontWeight: 600, fontSize: 14, marginBottom: 2}}>Player slots are full</div>
                  <div className="muted" style={{fontSize: 12, lineHeight: 1.4}}>You can still join as a spectator — watch the chaos in real time. If a player drops, you'll be offered the seat.</div>
                </div>
              </div>
            ) : (
          ...
          <button className="btn btn-ghost btn-block btn-sm" style={{marginTop: 8}} onClick={() => go("home")}>
            Cancel
          </button>
        </div>
      </div>
